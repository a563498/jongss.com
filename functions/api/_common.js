...
async function lookupWord(env, word){
  ...
  if ((!out.definition||!out.pos) && out.target_code){
    ...
    if (v.ok){
      ...
      const sense = item2?.senseinfo || item2?.senseInfo || item2?.sense || item2?.senseInfoList;
      const def = item2?.definition || sense?.definition || (Array.isArray(sense)? sense[0]?.definition : undefined);
      const pos = item2?.pos || sense?.pos || (Array.isArray(sense)? sense[0]?.pos : undefined);
      if (!out.definition && def) out.definition = def;
      if (!out.pos && pos) out.pos = pos;
    }
  }
  if (!out.definition) return {ok:false,status:502,message:"뜻풀이를 가져오지 못했어요(사전 응답 형식 문제).", detail:"search/view 응답에 definition이 비어있음. 다른 단어로도 동일하면 API 제한/응답 변경 가능"};
