...
async function pickDailyAnswer(env, gameId=null){
  ...
  // 하루 1회 생성이므로, 안정적으로 “정답+뜻풀이”가 나올 때까지 제한된 재시도
  for (let attempt=0; attempt<6; attempt++){
    const q = syllables[h % syllables.length];
    const startNo = 1 + ((h >>> 8) % 800);

    // include 검색으로 후보 단어들만 확보 (num은 최소 10)
    ...
    // 후보 중 일부만 lookup(뜻풀이 확보) — 최대 5개
    const offset = (h >>> 16) % arr.length;
    const limit = Math.min(5, arr.length);
    ...
    for (let k=0; k<limit; k++){
      ...
      // lookupWord는 exact 검색 + (가능하면) view 보강까지 포함 → definition 확보 확률이 높음
  ...
  return { ok:false, status: 502, message: "정답 뜻풀이를 가져오지 못했어요.", detail: "OpenDict에서 definition이 정상적으로 내려오지 않거나, 호출 제한/업스트림 오류가 반복되었습니다. /api/diag로 OpenDict 오류 메시지를 확인하세요." };
}
