...
export function getOpenDictReferer(env){
  const r = env && (env.OPENDICT_REFERER || env.USE_URL || env.OPENDICT_USE_URL);
  if (!r) return null;
  try{
...
function extractItems(xml){
  // Robustly extract <item ...>...</item> blocks without relying on regex matchAll
  const out = [];
  const s = String(xml||"");
  let i = 0;
  while (true){
    const a = s.indexOf("<item", i);
    if (a === -1) break;
    const openEnd = s.indexOf(">", a);
    if (openEnd === -1) break;
    const b = s.indexOf("</item>", openEnd);
    if (b === -1) break;
    out.push(s.slice(openEnd+1, b));
    i = b + 7;
...
async function fetchXml(url, envRef){
  try{
        const ref = getOpenDictReferer(envRef);
...
export async function lookupWord(env, word){
  ...
  if (!keyRes.ok) return { ok:false, status:500, message:keyRes.message };

  // 1) SEARCH: get a few candidates
  ...
  // retry 2 times if HTML/system-error
  let sres = null;
  for (let i=0;i<3;i++){
    sres = await fetchXml(u.toString(), env);
    const head = (sres.text||"").slice(0,60);
    if (sres.ok && head.includes("<?xml")) break;
  }

  if (!sres || !sres.ok){
    return { ok:false, status:(sres && sres.status) || 502, message:"사전 검색 실패", detail:(sres && sres.text ? sres.text.slice(0,260) : null) };
  }
  if (!((sres.text||"").includes("<?xml"))){
    return { ok:false, status:502, message:"사전이 XML이 아닌 HTML로 응답했어요(차단/오류 가능).", detail:(sres.text||"").slice(0,260) };
  }
  if (/<error>/i.test(sres.text)){
    return { ok:false, status:502, message:"사전 오류 응답", detail:sres.text.slice(0,260) };
  }

  // 후보들에서 정확히 일치하는 표제어 우선, 없으면 첫 후보
  ...
  // 2) VIEW: definition은 view에서 확실히 가져온다(검색 응답에서 비는 경우가 많음)
  let definition = item.definition || "";
  if (!definition){
    const tc = item.target_code;
    if (tc){
      const v = new URL(OPENDICT_VIEW);
      v.searchParams.set("key", keyRes.key);
      v.searchParams.set("target_code", tc);
      v.searchParams.set("req_type", "xml");
      const vres = await fetchXml(v.toString(), env);
      if (vres.ok && (vres.text||"").includes("<?xml") && !/<error>/i.test(vres.text)){
        const vitem = parseView(vres.text);
        if (vitem && vitem.definition) definition = vitem.definition;
  ...
  if (!definition){
    return { ok:false, status:502, message:"뜻풀이를 가져오지 못했어요(사전 응답 형식 문제).", detail:{ q, parsed:{ word:item.word, target_code:item.target_code }, searchSnippet: sres.text.slice(0,260) } };
  }

  const data = { word:item.word, pos:item.pos||"", definition, target_code:item.target_code||null, type:"", cat:"" };
  if (data.__nonExact) data.__nonExact = true;

  if (kv){
    await kv.put(cacheKey, JSON.stringify(data), { expirationTtl: 60*60*24*7 }).catch(()=>{});
  }
