...
## (중요) 우리말샘 사용 URL 체크가 있는 경우
우리말샘(OpenDict) 쪽에서 "사용 URL"을 검사하는 환경이면, Cloudflare에서 서버 호출 시 **Referer/Origin이 비어** 차단될 수 있어요.

이 경우 Pages → Settings → Variables and Secrets(Production)에 아래를 추가하세요.
- `OPENDICT_REFERER` = `https://<너의프로덕션도메인>/`

그 다음 `/api/raw?word=사과`에서 `isXml:true`가 나오는지 확인.
